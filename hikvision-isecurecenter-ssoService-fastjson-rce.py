...
if len(m) != 1 or len(m[0]) != 2:
    print('error: url<%s> parse fail' % url)
    sys.exit(0)
...
rsp = requests.post(url=url, headers=headers, data=data, proxies=proxies, verify=False)
